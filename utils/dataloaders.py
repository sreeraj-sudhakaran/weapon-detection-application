...
class LoadImages_direct:
    # YOLOv5 image/video dataloader, i.e. `python detect.py --source image.jpg/vid.mp4`
    def __init__(self, source_array, img_size=640, stride=32, auto=True):
    ...
    def __next__(self):
        if self.count == self.nf:
            raise StopIteration
        self.count += 1
        img0 = self.pic#source_array#cv2.imread(path)  # BGR
